...
void G_BeginRecording (void)
{
  ...
  if (num_extensions > 0)
  {
    ...
    *demo_p++ = prdemosig[2]; // 'B'
    *demo_p++ = prdemosig[3]; // 'M'
    *demo_p++ = prdemosig[4]; // '+'
    *demo_p++ = '\0';
    // extension version
    *demo_p++ = 1;
...
const byte* G_ReadDemoHeaderEx(const byte *demo_p, size_t size, unsigned int params)
{
  ...
  if (demover == 255)
  {
    ...
    // so the format is
    // demover byte == 255
    // "PRBM+" signature (w/ ending null terminator)
    ...
    // we check for the PRBM+ signature as mentioned.
    // MBF and Eternity Engine also use 255 demover, with other signatures.
    if (strncmp((const char *)demo_p, prdemosig, 5) != 0)
    {
      I_Error("G_ReadDemoHeader: Extended demo format 255 found, but \"PRBM+\" string not found.");
    }
